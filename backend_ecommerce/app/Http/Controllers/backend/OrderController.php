...
class OrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $order = $query->where([['status', '=', 1]])->with('order_detail')->orderBy('id', 'desc')->paginate($limit, ['*'], 'page', $page);
    ...
    public function destroy(string $id)
    {
        if ($id) {
            $order = Order::find($id);
            if ($order) {
                $order->order_detail->each(function ($item) {
                    $item->delete();
                });
                $order->delete();
            } else {
        ...
        return response()->json(['status' => 200, 'message' => "delete success", 'data' => $order]);
    }
}
