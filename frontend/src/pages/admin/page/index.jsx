...
const PageAdmin = () => {
  ...
  const handleDeleteClick = async (id) => {
    ...
    if (res.data.data.status == 200) {
      setDeleteId(id);
      toast.success("Xoá thành công");
    } else {
      toast.error("Xoá thất bại sản phẩm vẫn tồn tại trong danh mục");
    }
  };
  const handleStatusClick = async (id) => {
    await pageApi.status(id);
    setStatus(Math.random());
    toast.success("Thay đổi trạng thái thành công");
  };
  const { data, loading } = useApiCall(
    async () => {
      return await pageApi.getAll({ ...filter });
    },
    ...
    <div className="m-5">
      ...
      <div className="bg-white p-8 rounded-md w-full">
        <div className=" flex items-center justify-between pb-6">
          <div>
            <h1 className="text-gray-600 font-semibold">Trang đơn</h1>
            <span className="text-xs">All page item</span>
          </div>
